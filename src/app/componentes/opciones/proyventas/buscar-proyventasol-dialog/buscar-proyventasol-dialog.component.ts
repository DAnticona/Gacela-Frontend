...
export class BuscarProyventasolDialogComponent implements OnInit {
	...
	constructor(
		public dialogRef: MatDialogRef<BuscarProyventasolDialogComponent>,
	...
	filtrar(codigo: string, noFile: string, fechaIni: string, fechaFin: string, fgActi: string) {
		...
		this.archivosFiltrados = this.archivos.filter(a => {
			if (fgActi !== '99') {
				return (
					a.coFile.includes(codigo.toUpperCase()) &&
					a.noFile.includes(noFile.toUpperCase()) &&
					a.fgActi.includes(fgActi) &&
					a.feCargaFile.getTime() >= new Date(fechaIni).getTime() &&
					a.feCargaFile.getTime() <= new Date(fechaFin).getTime()
				);
			}
		});
