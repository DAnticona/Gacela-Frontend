...
export class ForecastComponent {
	...
	constructor(
		...
		this.servicioService.getServicios(this.token, this.urls).subscribe((ser: any) => {
			this.servicios = ser.body.servicios;

			this.navesService.obtieneNaves(this.token, this.urls).subscribe((nav: any) => {
				this.naves = nav.body.naves;

				this.forecastCab.coServ = this.servicios[0].coServ;
				this.forecastCab.fgProp = 'S';
				this.filtroNaves = this.naves.filter(
					nave =>
						nave.servicio.coServ === this.forecastCab.coServ && nave.linea.fgProp === this.forecastCab.fgProp
				);

				this.forecastCab.coNave = this.filtroNaves[0].coNave;
	...
	onChangeService() {
		...
		this.filtroNaves = this.naves.filter(
			nave =>
				nave.servicio.coServ === this.forma.controls.coServ.value &&
				nave.linea.fgProp === this.forma.controls.fgProp.value
		);

		if (this.filtroNaves.length > 0) {
			this.forma.controls.coNave.setValue(this.filtroNaves[0].coNave);
		} else {
	...
	search(naveSearch: string) {
		this.filtroNaves = this.naves.filter(nave => {
			return (
				(nave.noNave.includes(naveSearch.toUpperCase()) || nave.alNave.includes(naveSearch.toUpperCase())) &&
				nave.servicio.coServ === this.forma.controls.coServ.value &&
				nave.linea.fgProp === this.forma.controls.fgProp.value
			);
		});
	...
	validaData(data: Array<ForecastDet>) {
		data.forEach((row, index) => {
			let servicioSeleccionado: any = this.servicios.filter(
				s => s.coServ === this.forma.controls.coServ.value
			)[0];
			...
			if (this.pod) {
				if (!row.pod) {
					...
					servicioSeleccionado.puertos.filter(puer => puer.coIso === row.pod.toUpperCase()).length === 0
				) {
					this.errorFile.push(
						`Fila ${index + 2} Columna POD: puerto no válido para el servicio seleccionado`
					);
				} else {
			...
			if (this.type) {
				if (!row.type) {
					...
					!(
						row.type.toUpperCase() === 'DR' ||
						row.type.toUpperCase() === 'ST' ||
						row.type.toUpperCase() === 'PL' ||
						row.type.toUpperCase() === 'FC' ||
						row.type.toUpperCase() === 'OT' ||
						row.type.toUpperCase() === 'FO' ||
						row.type.toUpperCase() === 'RH' ||
						row.type.toUpperCase() === 'HC' ||
						row.type.toUpperCase() === 'SD' ||
						row.type.toUpperCase() === 'FR' ||
			...
			if (this.cnd) {
				if (!row.cnd) {
					...
					this.errorFile.push(
						`Fila ${index + 2} Columna CND: Contiene datos inválidos, solo se permiten F o E`
					);
				} else {
			...
			if (this.vgm) {
				if (!row.vgm) {
					this.errorFile.push(
						`Fila ${index + 2} Columna VGM(KG): Celda vacía, con valor 0 ó datos no válidos`
					);
	...
	onProcesar() {
		...
		let nave = this.naves.filter(n => n.coNave === this.forecastCab.coNave)[0];
		let servicio = this.servicios.filter(s => s.coServ === this.forecastCab.coServ)[0];

		let forecast = {
			coForecast: '',
			nave,
			servicio,
			detalle: this.forecastCab.detalle,
			fgProp: this.forecastCab.fgProp,
		};
		...
		this.forecastService.loadFile(this.urls, forecast, this.token).subscribe(
			(res: any) => {
				// console.log(res);
