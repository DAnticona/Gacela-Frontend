...
import { DialogCorregirCutOffComponent } from '../modales/dialog-corregir-cut-off/dialog-corregir-cut-off.component';
...
export class FileMtc1r999Component {
	...
	/**
	 * Variable que controla si se muestran los conflictos de cutt off
	 */
	...
	/**
	 * Variable que almacena la lista de conflictos
	 * de cutt off encontrados en el file solo de naves
	 * que estan registradas en la BD.
	 */
	protected conflictosEta: any = [];
	...
	private obtieneFileActivo() {
		...
		this.fileService.getFileActivo(this.token, this.urls).subscribe((res: any) => {
			this.fileMtc1r999 = res.body.file;
	...
	protected cargarFile(fileList: FileList) {
		...
		if (f) {
			...
			fileReader.onload = e => {
				...
				this.obtieneConflictosEta();

				// if (this.conflictosEta.lenght === 0) {
				// 	this.fileService.registrarFile(this.token, this.urls, this.fileMtc1r999).subscribe(
				// 		(res: any) => {
				// 			this.obtieneFileActivo();
				// 		},
	...
	private obtieneConflictosEta() {
		...
		this.navesService.obtieneNaves(this.token, this.urls).subscribe((res: any) => {
			...
			this.fileMtc1r999.detalle.sort((a, b) =>
				(a.nave + a.viaje + a.cut_off).localeCompare(b.nave + b.viaje + b.cut_off)
			);

			let naveViajeKey = this.fileMtc1r999.detalle
				.filter(f => f.booking_no.substr(0, 2) === '75')
				.map(x => x.nave + x.viaje)
				.filter((valor, indiceActual, arreglo) => arreglo.indexOf(valor) === indiceActual);

			for (let nvk of naveViajeKey) {
				let lista = this.fileMtc1r999.detalle.filter(f => f.nave + f.viaje === nvk);
				let lista2 = lista
					.map(x => x.cut_off.getTime())
					.filter((valor, indiceActual, arreglo) => arreglo.indexOf(valor) === indiceActual);
				let conflicto: any = {};
				conflicto.cutOffs = [];

				if (lista2.length > 1) {
					conflicto.nave = lista[0].nave;
					conflicto.viaje = lista[0].viaje;

					for (let l2 of lista2) {
						let cutOff = l2;
						conflicto.cutOffs.push(new Date(cutOff));
					}
					conflictos.push(conflicto);
				}
			}
			// console.log(conflictos);
			this.conflictosEta = conflictos;
			// console.log(this.conflictosEta);
			// console.log(this.fileMtc1r999.detalle);
			if (this.conflictosEta.length === 0) {
				this.cargando = true;
				this.enviaCargando.emit(this.cargando);
				// console.log(this.fileMtc1r999);
				this.fileService.registrarFile(this.token, this.urls, this.fileMtc1r999).subscribe(
					(res1: any) => {
						// console.log('exito');
						this.obtieneFileActivo();
	...
	corregirCutOff(index: number) {
		const dialogRef = this.dialog.open(DialogCorregirCutOffComponent, {
			width: '800px',
			height: '400px',
			data: {
				nave: this.conflictosEta[index].nave,
				viaje: this.conflictosEta[index].viaje,
				cutOffs: this.conflictosEta[index].cutOffs,
			},
		});

		dialogRef.afterClosed().subscribe(result => {
			// console.log(result);
			for (let i = 0; i < this.fileMtc1r999.detalle.length; i++) {
				if (
					this.fileMtc1r999.detalle[i].nave === this.conflictosEta[index].nave &&
					this.fileMtc1r999.detalle[i].viaje === this.conflictosEta[index].viaje
				) {
					// console.log(result);
					// console.log(result.substr(0, 4));
					// console.log(result.substr(5, 2));
					// console.log(result.substr(8, 2));
					// console.log(new Date(result.substr(0, 4), result.substr(5, 2), result.substr(8, 2)));
					this.fileMtc1r999.detalle[i].cut_off = new Date(
						Number(result.substr(0, 4)),
						Number(result.substr(5, 2)) - 1,
						Number(result.substr(8, 2))
					);
				}
			}
			this.obtieneConflictosEta();
