...
export class RatioDevolucionComponent {
	...
	constructor(private paramsService: ParamsService, private ratioDevolucionService: RatioDevolucionService) {
		this.urls = this.paramsService.urls;
		this.token = paramsService.conexion.token;

		this.forma = new FormGroup({
			coRatio: new FormControl(null),
	...
	private obtieneRatioDevolucion() {
		...
		this.ratioDevolucionService.obtieneRatio(this.token, this.urls).subscribe((res: any) => {
	...
	protected guardarRatio() {
		...
		this.ratioDevolucionService.registraRatio(this.token, this.urls, this.ratioDevolucion).subscribe(res => {
			this.obtieneRatioDevolucion();
		});
	}
}
